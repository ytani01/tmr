...
- [ ] Task: `get_str` のレンダリング計算（正常系）の TDD 実装
    - [ ] 0%, 50%, 100% の進捗に対する失敗テストの記述
    - [ ] テストを実行し、失敗を確認
    - [ ] 既存の実装でテストがパスすることを確認
- [ ] Task: 境界条件およびエッジケースの TDD 実装
    - [ ] 負の値、合計値超過、total=0 に対する失敗テストの記述
    - [ ] テストを実行し、失敗を確認
    - [ ] テストがパスすることを確認（必要に応じてロジック修正を検討）
- [ ] Task: レスポンシブな `bar_len` 動的変更の TDD 実装
    - [ ] 様々な `bar_len` 値に対する失敗テストの記述
    - [ ] テストを実行し、失敗を確認
    - [ ] テストがパスすることを確認
