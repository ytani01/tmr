...
タイトルは `TimerTitle(text, color, width)` で渡します。
`width` が正なら、その桁に揃えて表示します
（ポモドーロがフェーズ名を 16 桁で並べるのに使っています。
`pomodoro.py` の `TITLE_WIDTH`）。


## == キー割り当て

`Timer.cmd_list()` が返す `TimerCmd` のリストが、キー操作のすべてを
決めます。`__init__` がこれを `key_map = {key: fn}` に展開し、
メインループは押されたキーで引くだけです。
...
- キーを増やすときは、該当エントリの `keys` に足すだけです。
  `[?]` のヘルプ（`fn_help()`）は `cmd_list()` を読むので、
  自動で追随します
- キー名は blessed の `inkey().name`（`KEY_LEFT` など）か、
  大文字にした 1 文字です（`get_key_name()` が小文字を大文字にします）
- 表示用の整形は `KEY_WORD_MAP` と `keys_str()` です
  （`KEY_CTRL_B` → `[Ctrl]+[B]`、`KEY_LEFT` → `[←]`）
- `next` の行だけは `enable_next` を見て、単純タイマーでは
  ヘルプに出しません
...
アラームは daemon スレッドで鳴らします。**止め方は 2 通りあり、
`AlarmParams.cmd` の有無で分かれます。**

- **ベルの経路**（`cmd` なし）— スレッドが `\a` を鳴らし、
  停止の合図は `alarm_active` フラグ 1 つ。スレッド側もメインループ側も
  これを見ていて、鳴り終わればスレッドが自分で False にします
- **コマンドの経路**（`cmd` あり）— スレッドは `alarm_active` を見ず、
  コマンドの終了を待つだけです。止めるのはメインループ側で、
  キー入力の後に `stop_alarm_cmd()` がプロセスへシグナルを送ります（下記）
...
セクション名は**サブコマンドの正式名**です（別名の `t` / `p` は
書けません）。`default_map` は別名にも同じ dict を張るので、
`tmr t` でも `[timer]` が効きます。キーはオプション名から `--` を
取ったもので、`-` と `_` のどちらでも書けます。`timer` の `minutes` は
引数ですが、これも `default_map` の対象です（書いておけば省略できます）。

ファイルが無ければ黙って既定値を使い、**壊れていれば
`ClickException` で終了します**（TOML の構文エラー、知らないセクション、
知らないキー）。値の型が合わなければ `click` が弾きます。
